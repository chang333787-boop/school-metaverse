...
- 길격자 164,002칸(0.3m) · 도달성 100구역 통과 · 지도 계약 0(구역 100 · 지점 305 · 구역 없는 칸 0.9%).
- 🩺 빠른 판 10/10: 갇힘 0 · 옥상 0 · 문 그래프 0 · 지점 0 · 울타리 밖 0.
- 전체 판 **17/17**(09-26 main 16/17): 보이지 않는 벽 **15 → 0** · ghost **42 → 13** · 발 묻힘 **75 → 8** · 떠 있음 53 · 올라앉는 곳 14 → 13 · 계단 턱 530 → 524 · 구역 겹침 0 · 카메라 벽 뚫림(기본 피치) 0.06% · 문 직진 끼임 **1 → 0**(로비 자동문).
...
- **유치원 조합놀이대**: 노랑 터널이 눕힌 원기둥 기준점(밑면에서 −x로 뻗음) 탓에 탑 B 발판을 0.4 덮던 것 → 제 발판(cx+1.0 ±0.4) 위로(모습 0.4 이동) + 터널 막이 ·
...
- 지도 계약: '유치원 놀이터'(yr −2.55~0.6)·'놀이터'(−2.55~1.0) 높이 띠에 놀이대·미끄럼틀 발판 포함(예전 구역 없음) · `health.js` BASELINE = 이 실측(ghost 13 · sunk 8 · floating 53 · perch 13 · tallStep 524).
...
- 유치원 탑 A 지붕(밑 1.25 = A 발판 + 1.4)이 탑 B 발판(0.15) 남쪽 0.3을 덮음 → B 발판에 선 몸(1.5)의 머리가 지붕 속 ghost 3. 머리 막이는 A→B 걸음에서 B 높이로 올라서자마자
